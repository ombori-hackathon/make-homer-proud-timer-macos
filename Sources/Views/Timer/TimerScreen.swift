...
struct TimerContentView: View {
    ...
    var body: some View {
        ZStack {
            // Dark themed background with god accent gradient
            currentTheme.darkBackgroundGradient
                .ignoresSafeArea()

            // Geometric pattern overlay (darker for dark mode)
            GeometricPatternView(theme: currentTheme, opacity: 0.03)

            // Main content
            VStack(spacing: 20) {
                // Top bar with god selector and stats
                HStack(alignment: .top) {
                    // God quick selector (icon + name + dropdown)
                    GodQuickSelectorButton(
                        god: timerService.currentGod,
                        theme: currentTheme,
                        onGodSelected: { god in
...
/// Stats widget with god theme colors (dark mode)
...
/// Session type indicator with theme (dark mode)
            ...
            ForEach([SessionType.focus, .shortBreak, .longBreak], id: \.self) { type in
                Button {
                    ...
                    Text(type.displayName)
                        ...
                        .foregroundStyle(sessionType == type ? Color.white : HadesTheme.primaryText)
                        .padding(.horizontal, 12)
                        .padding(.vertical, 6)
                        .background(
                            RoundedRectangle(cornerRadius: 8)
                                .fill(sessionType == type ? theme.primaryColor : HadesTheme.tartarusGray)
                        )
                        .overlay(
                            RoundedRectangle(cornerRadius: 8)
                                .stroke(
                                    sessionType == type ? Color.clear : HadesTheme.asphodelusGray,
                                    lineWidth: 1
                                )
                        )
                }
...
/// Circular progress with theme colors (dark mode)
struct ThemedCircularProgress: View {
    ...
    var body: some View {
        ZStack {
            // Background circle
            Circle()
                .stroke(HadesTheme.asphodelusGray, lineWidth: 12)
...
/// Timer controls with theme (dark mode)
        ...
        HStack(spacing: 20) {
            ...
            Button(action: onReset) {
                Image(systemName: "arrow.counterclockwise")
                    .font(.title2)
                    .foregroundStyle(HadesTheme.primaryText.opacity(0.7))
                    ...
                    .overlay(
                        Circle()
                            .stroke(HadesTheme.asphodelusGray, lineWidth: 1)
                    )
            }
            .buttonStyle(.plain)
                ...
                Image(systemName: state == .running ? "pause.fill" : "play.fill")
                    ...
                    .background(
                        Circle()
                            ...
                            .shadow(color: theme.primaryColor.opacity(0.5), radius: 12, x: 0, y: 4)
                    )
            }
            .buttonStyle(.plain)
